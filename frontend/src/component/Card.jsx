      ...
      {stockinfo.map((stockdetails, index) => {
        ...
        return (
          <div className={`rounded-2xl min-w-xl w-auto max-w-3xl h-auto p-3 bg-linear-to-r ${uptrend ? 'from-[#112732c4] via-[#112732a2] to-[#11273269]' : 'from-[#321a1ac4] via-[#321a1aa2] to-[#321a1a69]'}`} key={index}>
            <div className='w-full flex justify-between text-[#10B981] text-sm'><h2 className='flex gap-2'>{uptrend ? <TrendingUp className="text-[#10B981]" size={20} /> : <TrendingDown className="text-red-500" size={20} />} <span className={uptrend ? "text-[#10B981]" : "text-red-500"}>{uptrend ? "+" : "-"}NPR {priceDifference} ({percentage}%)</span></h2></div>
